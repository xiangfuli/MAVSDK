...
if [[ "$ARCH" == "x86_64" ]]; then
  ...
  export PKG_CONFIG_PATH=$project_dir/build/default/third_party/install/lib/pkgconfig:$PKG_CONFIG_PATH
  export OPENSSL_ROOT_DIR=$project_dir/build/default/third_party/install/lib
fi
...
if [ ! -d $project_dir/build/default/install ]; then
  ...
  while  [ $attempt -lt $max_attempt ]; do
      cmake -DBUILD_TESTS=OFF -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DBUILD_MAVSDK_SERVER=OFF -DCMAKE_INSTALL_PREFIX=$project_dir/build/default/install -DCMAKE_MODULE_PATH="$project_dir/build/default/install/lib/cmake;$project_dir/build/default/third_party/install/lib/cmake;$project_dir/build/default/third_party/install/lib64/cmake" -Bbuild/default -S. && break 
